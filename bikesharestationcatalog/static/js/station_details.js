...
window.onload = function () {
    ...
    var day_string = weekday[day_num];
    ...
    var time_labels = labels.map(function(label) { return moment(label, 'HH:mm'); });
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            labels: time_labels,
            datasets: [{
                data: window.station_averages_json[day_string]
            }]
        },
        options: {
            scales: {
            ...
            tooltips: {
                ...
                callbacks: {
                    title: function(tooltipItems, data) {
                        return(data.labels[tooltipItems[0].index].format('HH:mm'));
                    },
                    label: function(tooltipItem, data) {
                        return (data.datasets[0].data[tooltipItem.index] + ' bikes')
    ...
    function updateDay(n) {
        ...
        myChart.data.datasets[0].data = window.station_averages_json[day_string];
        myChart.options.title.text = day_string;
        myChart.update();
    }

    document.getElementById('previousDayArrow').addEventListener('click', previousDay);
    document.getElementById('nextDayArrow').addEventListener('click', nextDay);
};
